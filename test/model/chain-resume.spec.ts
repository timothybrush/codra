...
describe('model chain resume', () => {
  ...
  async function review(service: ModelService) {
    ...
    } as Parameters<ModelService['reviewFile']>[0]);
  }
  ...
  it('resumes at the model after the ones that already failed, instead of replaying them', async () => {
    ...
    const first = new ModelService(env, tracker, { jobId: 'job-chain-resume' });
    ...
    const second = new ModelService(env, undefined, { jobId: 'job-chain-resume' });
  ...
  it('does not record progress past a model that was only rate-limited', async () => {
    const env = createTestEnv();
    await saveTestProviderApiKey(env);
    const service = new ModelService(env, undefined, { jobId: 'job-chain-429' });
  ...
  it('stops the chain the moment the invocation runs out of subrequests', async () => {
    const env = createTestEnv();
    await saveTestProviderApiKey(env);
    const service = new ModelService(env, undefined, { jobId: 'job-subrequests' });
