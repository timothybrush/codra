  ...
  return (
    <NavLink
      ...
      className={cn(
        ...
        'group flex h-[2.375rem] w-full items-center gap-3 rounded-md px-3 text-sm font-medium',
...
const collapsedTooltipClass = [
  ...
  'lg:z-50 lg:w-max lg:max-w-44 lg:rounded-md',
...
function CollapsedNavItem({
  ...
  return (
    <NavLink
      ...
      className={cn(
        'dashboard-sidebar-action',
        'group relative flex h-[2.375rem] w-[2.375rem] items-center justify-center rounded-md',
        'outline-none transition-[color,box-shadow] duration-200 ease-[var(--ease-out-quart)]',
        'focus-visible:ring-2 focus-visible:ring-ring',
        isActive ? 'bg-[oklch(79%_0.23_115/0.23)]' : '',
      )}
...
export function AppShell() {
  ...
  // Collapsed layout is desktop-only — the mobile drawer always shows the
  // full expanded sidebar, regardless of the persisted desktop preference.
  const collapsed = sidebarCollapsed && isDesktop;
  const accountMenuBesideSidebar = collapsed;
    ...
    <div className="flex min-h-svh bg-background" style={shellStyle}>
      ...
      >
        ...
        <div className={cn(
          ...
          'justify-between',
          collapsed && 'lg:flex-col lg:items-center lg:justify-start lg:gap-2 lg:pb-4',
        )}>
          ...
          >
            <img
              ...
              alt=""
              className={cn(
                'hidden h-8 w-8 shrink-0 rounded-md lg:block',
                !collapsed && 'lg:hidden',
              )}
            />
            <img
          ...
          <button
            ...
            className={cn(
              'hidden h-8 w-8 items-center justify-center rounded-md',
          ...
          <div className={cn('ml-auto flex items-center gap-1 lg:ml-0', collapsed && 'lg:hidden')}>
            <button
              onClick={() => setSidebarCollapsed(true)}
              className="hidden h-8 w-8 items-center justify-center rounded-md bg-white text-black shadow-sm transition-[background-color,color,transform] duration-200 hover:-translate-y-px hover:bg-[color-mix(in_oklch,var(--primary)_12%,white)] dark:bg-white/10 dark:text-white dark:hover:bg-white/15 lg:flex"
            ...
            <button
              onClick={toggleTheme}
              className="flex h-7 w-7 items-center justify-center rounded-md bg-white text-black shadow-sm transition-[background-color,color,transform] duration-200 hover:-translate-y-px hover:bg-[color-mix(in_oklch,var(--primary)_12%,white)] dark:bg-white/10 dark:text-white dark:hover:bg-white/15 lg:h-8 lg:w-8"
            ...
            <button
              onClick={() => setMobileMenuOpen(false)}
              className="flex h-7 w-7 items-center justify-center rounded-md bg-white text-black shadow-sm transition-colors hover:bg-secondary dark:bg-white/10 dark:text-white lg:hidden"
        ...
        {/* ── Footer ──────────────────────────────── */}
        <div className={cn('shrink-0 space-y-2 p-2 pt-3', collapsed && 'lg:flex lg:flex-col lg:items-center')}>
          ...
          <a
            ...
            title="Star on GitHub"
              className={cn(
                'dashboard-sidebar-action',
                'group relative flex h-[2.375rem] w-full items-center justify-center gap-2 rounded-md px-3',
                'bg-transparent',
              ...
              collapsed && 'lg:w-[2.375rem] lg:justify-center lg:px-0',
            )}
          >
            <Star
              size={14}
            ...
            />
            <span className={cn('dashboard-sidebar-action-label transition-transform duration-200 ease-[var(--ease-out-quart)]', collapsed && 'lg:hidden')}>
              Star on GitHub
            </span>
          </a>
              ...
              <DropdownMenuTrigger asChild>
                <button
                  ...
                  className={cn(
                    ...
                    collapsed && 'lg:h-8 lg:w-8 lg:justify-center lg:rounded-md lg:border-dashed lg:p-0',
                  )}
                >
                  <span className="relative shrink-0">
                    {sessionUser.avatarUrl ? (
                      ...
                      <span className={cn(
                        ...
                        collapsed && 'lg:h-7 lg:w-7 lg:text-xs',
                      )}>
                        {accountInitial}
                      </span>
                    )}
                  </span>
                  <span className={cn('dashboard-sidebar-action-label min-w-0 flex-1', collapsed && 'lg:hidden')}>
      ...
      <main className="flex min-w-0 flex-1 flex-col transition-[margin,color,background-color] duration-300 ease-[var(--ease-out-expo)] lg:ml-[calc(var(--app-sidebar-width)+2rem)]">
        ...
        <div className="app-shell-content mx-auto w-full max-w-screen-2xl px-4 py-6 md:px-6 md:py-8 lg:px-8">
          <Outlet />
        </div>
      </main>
    </div>
