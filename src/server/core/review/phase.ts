...
//
// The review phase: pick a chunk of files, dispatch them, settle the outcome, reschedule.
...
export async function runReviewPhase(
  ...
  // Cap concurrency by remaining budget, honouring the configured level in full when healthy.
  // Sized against the actual chain: a nine-model chain costs far more per file than a one-model one.
  ...
  // Single-threaded, so ++ is safe. terminalProgress: files that reached a terminal state this
  // chunk. awaitingAsync: files still queued on the async batch.
  let terminalProgress = 0;
  let awaitingAsync = 0;

  // Retry fast path: bulk-copy reusable parent reviews in one DB pass instead of one per budget
  // slot, so a fully-inheritable retry finishes in one invocation instead of ~12 hibernated chunks.
  ...
  // A file reached a terminal state, so clear the no-progress counter. A chunk that only
  // submitted or polled pending batches made none and must not reset it.
  ...
  if (rejected.length > 0) {
    ...
    // Surface a transient/subrequest-budget rejection as a single error so the orchestrator
    // reschedules the chunk, instead of failing the job with an AggregateError.
  ...
  if (completedCount >= files.length) {
    ...
    // Finalize needs its own fresh budget: TokenTracker under-reports usage (no Hyperdrive/GitHub
    // subrequests), so a conditional yield let finalize die with "Too many subrequests".
    await enqueueJobPhase(env, job.id, 'finalize', FRESH_INVOCATION_YIELD_SECONDS);
    return;
  }

  // Only in-flight batches left: poll after a delay. A pending-only chunk never resets the
  // continuation counter, so a batch that never completes eventually degrades to a partial review.
  ...
  if (job.checkRunId) {
    // Best-effort cosmetics only: reviews are already persisted, so a failure here (budget spent)
    // must not stop the next chunk from being enqueued.
    try {
  ...
  // More files remain: yield long enough to force hibernation into a fresh invocation, rather
  // than accumulating subrequests across chunks in this one.
