import { submitCloudflareBatch, pollCloudflareBatch } from '../models/cloudflare';
...
> {
  ...
  try {
    const poll = await ctx.rateLimits.runShared(() =>
      pollCloudflareBatch(ctx.env, resolved.modelName, params.requestId, ctx.tracker, resolved.providerName),
    );
